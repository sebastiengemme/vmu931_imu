...
class vmu931():
    def __init__(self, port, baudrate=9600):
        ...
        self.value = {
            "Accelerometers": dato.Dato(),
            "Gyroscopes": dato.Dato(),
            "Magnetometers": dato.Dato(),
            "Quaternions": dato.Dato(),
            "Euler": dato.Dato(),
            "Heading": dato.Dato(),
            "Text": dato.Dato()}
    ...
    def streamingAccelerometers(self, value):
        if (value and not self.status.streamingAcc) or (
                not value and self.status.streamingAcc):
            self.sendCommand(ACCELEROMETERS)

    def streamingGyroscopes(self, value):
        if (value and not self.status.streamingGyro) or (
                not value and self.status.streamingGyro):
            self.sendCommand(GYROSCOPES)

    def streamingMagnetometers(self, value):
        if (value and not self.status.streamingMag) or (
                not value and self.status.streamingMag):
            self.sendCommand(MAGNETOMETERS)

    def streamingQuaternions(self, value):
        if (value and not self.status.streamingQuart) or (
                not value and self.status.streamingQuart):
            self.sendCommand(QUATERNIONS)

    def streamingEulerAngles(self, value):
        if (value and not self.status.streamingEuler) or (
                not value and self.status.streamingEuler):
            self.sendCommand(EULER_ANGLES)

    def streamingHeading(self, value):
        if (value and not self.status.streamingHead) or (
                not value and self.status.streamingHead):
    ...
    def printStatus(self):
        print("Status devices: \n mag={} gyro={} acc={} \nResolution: \n gyro={} acc={} \nLow output rate status: {}\nStreaming: \n Head={} Uler={} Mag={} Quart={} Gyro={} Acc={}".format(
            self.status.magStatus,
            self.status.gyroStatus,
            self.status.accStatus,
            self.status.gyroResolution,
            self.status.accResolution,
            self.status.ouptputRate,
            self.status.streamingHead,
            self.status.streamingEuler,
            self.status.streamingMag,
            self.status.streamingQuart,
            self.status.streamingGyro,
            self.status.streamingAcc))
        return
